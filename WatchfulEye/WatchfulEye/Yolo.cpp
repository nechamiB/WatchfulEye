...
Mat Yolo::post_process(Mat& input_image, vector<Mat>& outputs, const vector<string>& class_list, double timestamp) {
	...
	for (int i = 0; i < ROWS; ++i)
	{
		...
		if (confidence >= CONFIDENCE_THRESHOLD)
		{
			...
			if (max_class_score > SCORE_THRESHOLD)
			{
				...
				float w = data[W];
				float h = data[H];
				int left = int((cx - NUMBER_FOR_ALIGNS * w) * x_factor);
				int top = int((cy - NUMBER_FOR_ALIGNS * h) * y_factor);
...
void Yolo::process_detected_results(Mat& input_image, const vector<string>& class_list, 
	const vector<int>& class_ids, const vector<float>& confidences, const vector<Rect>& boxes, double timestamp, vector<int> indices)
{
	for (int i = 0; i < indices.size(); i++)
	{
		...
		if(class_list[class_ids[idx]] == ITEMS_TO_DETECTION) {
			...
			csv.SaveDetectionDataToCSV(input_image, box, timestamp);
		}
	}
}
